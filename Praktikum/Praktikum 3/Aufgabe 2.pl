...
membertree(X,node(_,Lb,Rb)) :- membertree(X,Lb), binbaum(Lb),binbaum(Rb). %linken Baum durchsuchen
membertree(X,node(_,Lb,Rb)) :- membertree(X,Rb), binbaum(Lb),binbaum(Rb). %rechten Baum durchsuchen
